  ...
  <v-content>
    ...
    <v-container>
      <div align="center" v-if="selected">
        ...
        <p class="headline">Coronavirus cases in {{selected.country}}</p>
            
              <h3>Total: {{selected.cases.total}}</h3>
              <p class="overline blue--text">(New {{selected.cases.new}})</p>
              <h3>Death: {{selected.deaths.total}}</h3>
              <p class="overline red--text">(New {{selected.deaths.new}})</p>
    ...
    <v-row class="mx-auto">
      <v-col cols="12" md="6" class="px-0 d-flex flex-column">
        ...
        <chart v-if="loaded" :chart-data="datacollection2"></chart>
        </v-card>
      </v-col>
      <v-col cols="12" md="6" class="px-0">
        <v-card class="pt-4 pb-1 ml-1" height="100%">
        <PieChart v-if="loaded" :chart-data="chartData" :chart-option="chartOptions"></PieChart>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
...
export default {
      ...
      const datasets = [
        {
          ...
          data: [this.selected.cases.active, this.selected.cases.new, this.selected.deaths.total, this.selected.deaths.new]
        }
      ];
  ...
  methods: {
    async getCountry(){
      const host = 'https://2ii1cwtysk.execute-api.eu-central-1.amazonaws.com/default/covidTrackerProxy';
      const result = await axios.get(host)
      this.loaded = true;
      this.countryList = result.data.response;
